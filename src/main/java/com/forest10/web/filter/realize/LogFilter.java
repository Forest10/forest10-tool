...
public class LogFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        CachedHttpServletRequestWrapper httpServletRequestWrapper =
            new CachedHttpServletRequestWrapper(request, INIT_CACHE_LEN, MAX_CACHE_LEN);
        CachedHttpServletResponseWrapper httpServletResponseWrapper =
            new CachedHttpServletResponseWrapper(response, INIT_CACHE_LEN, MAX_CACHE_LEN);
        try {
    ...
    private void saveLogData(CachedHttpServletRequestWrapper requestWrapper,
        ...
        try {
            ...
            byte[] requestData = requestWrapper.getCachedStream()
                .getCached();
            byte[] responseData = responseWrapper.getCachedStream()
                .getCached();
            ...
            responseBody =
                responseBody.length() < LIMIT_BODY_LENGTH ? responseBody.replaceAll("\n|\r", "") : StringUtils.EMPTY;
            //打印 log
            log.info(
                String.format(LOG_FMT, method, url, userId, userName, requestHeader, requestBody, responseBody, time));
        } finally {
