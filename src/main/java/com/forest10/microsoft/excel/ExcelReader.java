package com.forest10.microsoft.excel;
...
public class ExcelReader {
     ...
     * @throws IOException
     */
    public LinkedHashMap<Integer, LinkedList<String>> readExcelContentSafeLy(String excelPath, InputStream inputStream)
        throws IOException {
    ...
    public LinkedHashMap<Integer, LinkedList<String>> readExcelContent(String excelPath, InputStream inputStream)
        ...
        if (isXLS) {
            ...
            for (int i = 1; i <= rowNum; i++) {
                ...
                int j = 0;
                while (j < colNum) {
                    if (Objects.nonNull(xFRow) && StringUtils.isNotBlank(
                        getCellFormatValue(row.getCell((short)j)).trim())) {
            ...
            for (int i = 1; i <= rowNum; i++) {
                ...
                while (j < colNum) {
                    if (Objects.nonNull(xFRow) && StringUtils.isNotBlank(
                        (getCellFormatValue(xFRow.getCell((short)j)).trim()))) {
    ...
    private String handleNUMERICCellValue(Cell cell) {
        ...
        if (HSSFDateUtil.isCellDateFormatted(cell)) {
            ...
            if (cell.getCellStyle()
                .getDataFormat() == HSSFDataFormat.getBuiltinFormat("h:mm")) {
